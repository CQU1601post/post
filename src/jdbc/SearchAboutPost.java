...
public class SearchAboutPost {
    ...
    public List<Ad> adsOfPost(int postId, int adTypeId) {
        ...
        if (adTypeId > 0) {
            ...
            // 返回通过审核的广告并且按时间排序
            String sql = "select * from ad where exist=1 and postId='" + postId
                    + "' and adTypeId='" + adTypeId + "'and checked=1 ";
            ResultSet result = connect.executeQuery(sql);
            ...
            while (result.next()) {
                ...
                if (remark.contains(text)) {// 如果广告中包含该字符串，则返回此广告粘贴栏
                    ...
                    for (int i = 0; i < posts.size(); i++) {
                        ...
                        for (Map.Entry<String, List<Post>> entry : map
                            ...
                            if (entry.getKey().equals(unitString)) {
                                judgeRepeat = true;
                                judgeExist = true;
                                for (Iterator iterator = entry.getValue()
                    ...
                    if (judgeRepeat) {
                        ...
                        List<Post> postList2 = new ArrayList<Post>();// 必须要重新new
                                                                     // 不然地址是相同的。
    ...
    public List<Map<String, Map<Integer, List<Ad>>>> adsContaintText1(
            String text) {
        // System.out.println("执行src/jdbc/SearchFromDB/adsOfPost(),传入的postId为："+postId);
        ConnectDB connect = new ConnectDB();
        ...
        try {
            while (result.next()) {
                ...
                if (remark.contains(text)) {// 如果广告中包含该字符串，则返回此广告粘贴栏
                    ...
                    if (judgeRepeat) {
                        Map<Integer, List<Ad>> recordMapValueMap2 = new HashMap<Integer, List<Ad>>();// 必须要重新new
                                                                                                     // 不然地址是相同的。
    ...
    public List<Ad> getAd(int postId, int money) {
        ConnectDB connectDB = new ConnectDB();
        String sql = "select * from ad where checked=1 and exist=1 and postId='"
                + postId + "'and money>'" + money + "'";
    ...
    public List<Ad> getAdByMoney(int money) {
        ConnectDB connectDB = new ConnectDB();
        String sql = "select * from ad where checked=1 and exist=1 and  money>'"
                + money + "'";
        ResultSet result = connectDB.executeQuery(sql);
        List<Ad> ads = new ArrayList<Ad>();
        try {
