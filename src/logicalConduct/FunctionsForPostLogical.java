...
public class FunctionsForPostLogical {
    ...
    public void unitsOfType(HttpServletRequest request,
        ...
        if (null == request.getParameter("unitTypeId")) {// 如果传来的参数信息不全，跳回主页
            ...
            int unitTypeId = Integer.parseInt(request
                    .getParameter("unitTypeId"));
            String unitTypeName = request.getParameter("unitTypeName");
            // unitTypeName=new
            // String(unitTypeName.getBytes("ISO-8859-1"),"UTF-8");
            System.out.println("unitTypeId=" + unitTypeId + "unitTypeName="
                    + unitTypeName);
            ...
            request.getRequestDispatcher("unitsAndPostsOfType.jsp").forward(
                    request, response);
        }
    ...
    public String getVisitorIp(HttpServletRequest request,
        ...
        if (visitorip == null || visitorip.length() == 0
                || "unknown".equalsIgnoreCase(visitorip)) {
    ...
    public void enterPost(HttpServletRequest request,
            HttpServletResponse response) throws ServletException, IOException {
        request.setCharacterEncoding("utf-8");
        response.setCharacterEncoding("utf-8");
        String page = "";
        ...
        if (null == request.getParameter("postId")
                ...
                || null == request.getParameter("unitTypeId")) {
            ...
            System.out.println("unitName=" + unit.getUnitName() + "postName="
                    + post.getPostName());
            ...
            String sql = "select * from visitorlog where visitorIP='"
            ...
            if (post.getUserId() <= 0) {// 如果不是专栏
                ...
                if (adTypeId == 0) {// adTypeId==0则返回所有广告
                    ...
                    ads = (List<PrivateAd>) searchFromDB
                            .adsOfPrivatePost1(postId);
                } else {// 如果adTypeId不为0则返回对应类别的广告
                // ads = (List<PrivateAd>) searchFromDB.adsOfPrivatePost(
                // postId, adTypeId, 0,
                // Configuration.PICNumOFEVERYLOADING);
                ...
                page = "privatePost.jsp";
                // try {
                // request.getRequestDispatcher("privatePost.jsp").forward(request,
                // response);
                // return;
                // } catch (Exception e) {
                // System.out.println(e.getMessage());
    ...
    public void adsOfPost(HttpServletRequest request,
            HttpServletResponse response) throws ServletException, IOException {
        request.setCharacterEncoding("utf-8");
        response.setCharacterEncoding("utf-8");
        response.setContentType("application/json;utf-8");
    ...
    public void picsOfAd(HttpServletRequest request,
            HttpServletResponse response) throws ServletException, IOException {
        request.setCharacterEncoding("utf-8");
        response.setCharacterEncoding("utf-8");
        Ad ad = new Ad();
        if (null == request.getParameter("adId")
                || null == request.getParameter("postId")) {
            ...
            vl.setTime(getTimestamp());
            vl.setAdId(adId);
            String sql1 = "select * from visitorlog where visitorIP='"
                    + vl.getVisitorip() + "'and postId='" + vl.getPostId()
                    + "'and adID='" + vl.getAdId() + "'";
            ...
            // 如果不是专栏
                ...
                // String sql = "update ad set click=click+1 where adId=" +
                // adId;
                // searchFromDB.updateClick(sql);
                privateColumn = "0";
            }
            ...
            request.getRequestDispatcher("picsOfAd.jsp").forward(request,
                    response);
        }
    ...
    public void unitsWithPublicPost(HttpServletRequest request,
        ...
        PrintWriter out = response.getWriter();
        System.out
                .println("执行:src/logicalConduct/FunctionsForPostLogical/unitsWithPublicPost");
        if (null == request.getParameter("unitTypeId")) {// 如果传来的参数信息不全，跳回主页
            System.out.println("您尚未选中任何单位类别，请在主页选择您要查看的单位类别");
            response.sendRedirect("index.jsp");
        } else {
    ...
    public void unitsWithPublicAD(HttpServletRequest request,
        ...
        PrintWriter out = response.getWriter();
        System.out
                .println("执行:src/logicalConduct/FunctionsForPostLogical/unitsWithPublicAD");
    ...
    public void publicPostsOfUnit(HttpServletRequest request,
        ...
        PrintWriter out = response.getWriter();
        System.out
                .println("执行:src/logicalConduct/FunctionsForPostLogical/publicPostOfUnit");
    ...
    public void searchPosts(HttpServletRequest request,
        ...
        response.setCharacterEncoding("utf-8");
        System.out
                .println("执行:src/logicalConduct/FunctionsForPostLogical/searchPosts");
        if (null == request.getParameter("searchText")) {// 如果传来的参数信息不全，跳回主页
            ...
            // searchText = new String(searchText.getBytes("iso-8859-1"),
            // "utf-8");
            ...
            request.getRequestDispatcher("searchedPosts1.jsp").forward(request,
                    response);
        }
    ...
    // 选择所有包含指定广告的单位和粘贴栏
    public void searchAds(HttpServletRequest request,
            HttpServletResponse response) throws ServletException, IOException {
        request.setCharacterEncoding("utf-8");
        response.setCharacterEncoding("utf-8");
        System.out
                .println("执行:src/logicalConduct/FunctionsForPostLogical/searchAds");
        if (null == request.getParameter("searchText")) {// 如果传来的参数信息不全，跳回主页
            System.out.println("您未输入任何搜索字段");
            response.sendRedirect("index.jsp");
        } else {
    ...
    public void enterUnit(HttpServletRequest request,
        ...
        request.setCharacterEncoding("utf-8");
        response.setCharacterEncoding("utf-8");
        if (null == request.getParameter("unitInfo")
                || null == request.getParameter("adTypeId")) {
            System.out.println("您尚未选择任何粘贴栏");
            response.sendRedirect("index.jsp");
        } else {
    ...
    public void addLastAdMoney(HttpServletRequest request,
            HttpServletResponse response) throws ServletException, IOException {
        request.setCharacterEncoding("utf-8");
        response.setCharacterEncoding("utf-8");
        ...
        try {
            ...
            if (fileItems.size() <= 0) {// 没有图片
                ...
                // 存储广告到对应的粘贴栏、类别
                    ...
                    saveAds1(postIds, adTypeId, remark, isPrivateAd, fileItems,
                            request, response);
                }
            }
    ...
    // 广告存储
    public void saveAds1(String[] postIds, int adTypeId, String remark,
            boolean isPrivateAd, List<FileItem> fileItems,
        ...
        System.out
                .println("isPrivateAd:" + request.getParameter("privatePost"));
        if ((user == null || user.getUserType() <= 0) && postIds.length > 1) {// 如果不是专栏用户但是选择了多个粘贴栏
             ...
             * 如果是专栏用户，可以上传到多个粘贴栏，广告加水印表示为专栏用户，checked=1；不是专栏用户
             * ，上传到一个粘贴栏，checked=0然后再将所有图片存到数据库
             * 用privatePost标示用户是不是上传到自己专栏，如果是的话，则将广告存储到专栏广告表
             */
            String compressPath = request.getSession().getServletContext()
                    .getRealPath("/firstPics").replace("/", "\\");// 首图压缩后存放路径
            String path = request.getSession().getServletContext()
                    .getRealPath("/photoes").replace("/", "\\");// 图片存储路径
            String TSPath = request.getSession().getServletContext()
                    .getRealPath("/TSphotoes").replace("/", "\\");// 图片存储路径
            ...
            if (firstPic.getSize() > 1024 * 1024 * 2) {
                new DisposePic().compress(TSPath + "\\" + firstPicPath,
                        compressPath + "\\" + firstPicCompressPath, 250, 250,
                        0.8F);
                new DisposePic().compress(TSPath + "\\" + firstPicPath, path
                        + "\\" + firstPicCompressPath, 1024, 1024, 1F);
                ...
                new DisposePic().compress(path + "\\" + firstPicPath,
                        compressPath + "\\" + firstPicCompressPath, 250, 250,
                        0.8F);
            }
            ...
            // 给压缩图加上标题
            // new
            // DisposePic().createMark(compressPath+"\\"+firstPicCompressPath,compressPath+"\\"+firstPicCompressPath,remark);
            ...
            if (isPrivateAd == true) {// 如果是专栏广告，广告存放在专栏广告表中,广告只会上传到一个粘贴栏-自己专栏
                ...
                PrivateAd ad = new PrivateAd(adId, adTypeId, upLoadTime,
                        userId, Integer.parseInt(postIds[0]), firstPicAddr,
                        money, sortValue, remark, 250, 250, 1, 0, 0);
                ...
                for (int i = 0; i < postIds.length; i++) {
                    ...
                    Ad ad = new Ad(id, adTypeId, upLoadTime, userId,
                            Integer.parseInt(postIds[i]), firstPicAddr, money,
                            sortValue, checked, remark, 250, 250, 0, 1, 0);
            ...
            for (int j = 0; j < fileItems.size(); j++) {// 存储该广告下所有图片
                ...
                if (fileItem.getSize() > 1024 * 1024 * 2) {
                    new DisposePic().compress(TSPath + "\\" + filedir, path
                            + "\\" + filedir, 1024, 1024, 1F);
                    fileflag = file.delete();
                ...
                if (isPrivateAd == true) {// 如果是专栏广告，广告下所有图片存放在专栏图片表中
                    ...
                    for (int i = 0; i < postIds.length; i++) {// 如果有多个粘贴栏，则在每个粘贴栏下申明一个对应的adId，而图片需要对应每一个广告
                        ...
                        Pic pic = new Pic(0, picAddr, width, height, checked,
                                id);
    ...
    // 广告存储
    public void saveAds(String[] postIds, int adTypeId, String remark,
            boolean isPrivateAd, List<FileItem> fileItems,
            HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        request.setCharacterEncoding("utf-8");
        ...
        if ((user == null || user.getUserType() <= 0) && postIds.length > 1) {// 如果不是专栏用户但是选择了多个粘贴栏
            ...
            // 压缩首图,并且存储在对应路径
            new DisposePic().compress(path + "\\" + firstPicPath, compressPath
                    + "\\" + firstPicCompressPath, 250, 250, 0.8F);
            // 给压缩图加上标题
            // new
            // DisposePic().createMark(compressPath+"\\"+firstPicCompressPath,compressPath+"\\"+firstPicCompressPath,remark);
            String upLoadTime = new GetCurrentTime().currentTime();// 获取当前时间，作为广告的上传时间
    ...
    public void getScrollAds(HttpServletRequest request,
        ...
        if (null == request.getParameter("num")
            ...
            int postId = Integer.parseInt(request.getParameter("postId"));
            System.out.println("postID" + postId);
            List<BrowserControl> browserControls = searchFromDB
                    .selectAllBrowerControl();
            int row = browserControls.get(0).getRow();
            List<Ad> ads = searchFromDB.getRandAd(postId, row, 0);
            List<String> fristPicList = new ArrayList<String>();
            if (ads != null) {
                for (Iterator iterator = ads.iterator(); iterator.hasNext();) {
                    Ad ad = (Ad) iterator.next();
